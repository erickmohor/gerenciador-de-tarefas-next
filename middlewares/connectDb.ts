...
export const connectDb = (handler : NextApiHandler) => 
    ...
    if(!DB_CONNECTION_STRING){
        return res.status(500).json({ error : "Database connection não informada no ENV."});
    }

    await mongoose.connect(DB_CONNECTION_STRING);

    mongoose.connection.on('connected', () => console.log('Conectado ao DB.'));
    mongoose.connection.on('error', error => console.log('Ocorreu um erro ao conectar no DB.' + error));
